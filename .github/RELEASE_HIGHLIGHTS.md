...
- **External-contract AMQP publishing** — `@connectum/events-amqp` `publisherOptions.externalContract` emits only contract-specified properties (no EventBus envelope), plus caller-set `PublishOptions.messageId`/`timestamp`, so you can publish to a third-party AsyncAPI/AMQP contract through the adapter's confirm/recovery machinery (#161). ([#185](https://github.com/Connectum-Framework/connectum/pull/185), [#186](https://github.com/Connectum-Framework/connectum/pull/186))
